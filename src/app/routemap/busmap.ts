//"use strict";
//これ以外に読み込みが必要なもの
import {CircleMarker, LatLng, Polyline, LeafletEvent, Map, LatLngBounds, Marker, bounds} from "leaflet";
...
export namespace RouteMAP{
  ...
  export class RouteMapMain{
    ...
    constructor(private routemapService:RoutemapService,private routes:{[key:string]:Route},private stations:{[key:string]:Station}) {
    }
    //ズームした時のオフセット量
    ...
    public  init(){
      ...
      for (let stationID in this.stations) {
        this.stations[stationID].nameMarker=L.marker([this.stations[stationID].lat, this.stations[stationID].lon],
          {"icon": L.divIcon({"html": this.stations[stationID].jptiStation.name, className: "className", iconSize: [50, 16], iconAnchor: [-4, -4]})}).addTo(this.stationNameLayer).on("click",()=>this.stationClicked(stationID));
      }
      this.f_zoom();
